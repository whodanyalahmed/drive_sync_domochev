...
def download_images(media_items):
    ...
    for x in media_items:
        filename = x['filename']
        # Read file picture taken date/time
        fd = x['mediaMetadata']['creationTime']
        ...
        # Create tuple time format to be used later
        ...
        # File is video type
            ...
            if 'READY' in video_status:
                ...
                print('*** ATTENTION: Video Status is not READY for ' +
                      filename + ' ***')
...
# Open file again to record file names as new files are downloaded
f = open("download_list.txt", "a+")

results = service.mediaItems().list(
    pageSize=100).execute()  # Get items from Google Photos
media_num, download_num, skip_num, error_num, new_download = download_images(
    results['mediaItems'])  # Down files
...
while True:
    results = service.mediaItems().list(
        pageSize=100, pageToken=next).execute()  # Get items from Google Photos
    if len(results) == 1:
        break
    media_num, download_num, skip_num, error_num, new_download = download_images(
        results['mediaItems'])  # Down files
